...
# Help Context
def helper():
        print("\n"+bcolors.OKGREEN+"                                     ╔════ CLI USAGE GUIDE ════╗"+bcolors.ENDC)
        print("\t./onixscanner.py example.com: Scan the domain example.com.")
        print("\t./onixscanner.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./onixscanner.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./onixscanner.py --update   : Update the scanner to the latest version.")
        print("\t./onixscanner.py --help     : Display this help message.")
        print("\n"+bcolors.OKGREEN+"                                     ╔════ CONTROL INSTRUCTION ════╗"+bcolors.ENDC)
        print("\t                                Ctrl+C: Skips current test.")
        print("\t                                Ctrl+Z: Quits OnixScanner.")
        print("\n"+bcolors.OKGREEN+"                                     ╔════ SCANNING PROCESS ════╗"+bcolors.ENDC)
        print("\t["+proc_high+"]: The scanning process might take longer than expected and its duration is uncertain")
        print("\t["+proc_med+"]: The scanning process might take less than 10 minutes to finish.")
        print("\t["+proc_low+"]: The scanning process might take just a minute or two..")
        print("\n"+bcolors.OKGREEN+"                                     ╔════ VULNERABILITY INFORMATION ════╗"+bcolors.ENDC)
        print("\t"+vul_info('c')+":This needs urgent attention because it could cause a security issue or make the service unavailable.")
        print("\t"+vul_info('h')+": It might not result in a quick agreement, but there is a good chance of it happening.")
        print("\t"+vul_info('m')+": An attacker could combine several vulnerabilities of this kind to carry out a complex attack.")
        print("\t"+vul_info('l')+": It’s not a big problem, but it’s a good idea to address the finding.")
        print("\t"+vul_info('i')+": This is not a vulnerability; it's just a helpful informational alert to keep in mind..\n")
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"Onix Scanner has detected multiple instances of Ctrl+C commands. Exiting now..." +bcolors.ENDC)
            sys.exit(1)
    ...
    def stop(self):
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"OnixScanner detected multiple Ctrl+C commands. Exiting now..." +bcolors.ENDC)
            sys.exit(1)
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("OnixScanner is updating....Please wait.\n")
    spinner.start()
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of OnixScan." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"OnixScan successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/Onixscan* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"OniScanner was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/onixscan_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit OnixScan.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write(vuln_info[arg2].replace('Onixscan', 'onixscanner'))
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/Onixscan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory OnixScan resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/Onixscan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/Onixscan_te* > /dev/null 2>&1') # Clearing previous scan files