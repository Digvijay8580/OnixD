...
def vul_info(val):
    result = ""
    if val == "c":
        result = BColors.BG_CRIT_TXT + " Critical⚠️ " + BColors.ENDC
    elif val == "h":
...
def helper():
    ...
    print("\t./OnixD.py example.com: Scan the domain example.com.")
    print(
        "\t./OnixD.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests."
    )
    print(
        "\t./OnixD.py example.com --nospinner: Disable the idle loader/spinner."
    )
    print("\t./OnixD.py --update   : Update the scanner to the latest version.")
    print("\t./OnixD.py --help     : Display this help message.")
    print(
        "\n"
        + BColors.OKGREEN
        + "                                     ╔════ CONTROL INSTRUCTION ════╗"
    ...
    print(
        "\t["
        + proc_high
        + "]: The scanning process might take longer than expected and its duration is uncertain"
    )
    print(
        "\t["
        + proc_med
        + "]: The scanning process might take less than 10 minutes to finish."
    )
    print(
        "\t[" + proc_low + "]: The scanning process might take just a minute or two.."
    )
    print(
        "\n"
        + BColors.OKGREEN
    ...
    print(
        "\t"
        + vul_info("c")
        + ":This needs urgent attention because it could cause a security issue or make the service unavailable."
    )
    print(
        "\t"
        + vul_info("h")
        + ": It might not result in a quick agreement, but there is a good chance of it happening."
    )
    print(
        "\t"
        + vul_info("m")
        + ": An attacker could combine several vulnerabilities of this kind to carry out a complex attack."
    )
    print(
        "\t"
        + vul_info("l")
        + ": It’s not a big problem, but it’s a good idea to address the finding."
    )
    print(
        "\t"
        + vul_info("i")
        + ": This is not a vulnerability; it's just a helpful informational alert to keep in mind..\n"
    )
    ...
    def spinner_task(self):
        ...
        try:
            while self.busy:
                if not self.disabled:
                    ...
                    if inc > random.uniform(0, terminal_size()):  # 30 init
                        print(end="\r")
                        BColors.BG_SCAN_TXT_START = (
                            "\x1b[6;30;" + str(round(random.uniform(40, 47))) + "m"
                        )
                        inc = 0
                    sys.stdout.flush()
            ...
            print(
                "\n\t"
                + BColors.BG_ERR_TXT
                + "OnixD has detected multiple instances of Ctrl+C commands. Exiting now..."
                + BColors.ENDC
            )
            sys.exit(1)
    ...
    def stop(self):
        try:
            ...
            print(
                "\n\t"
                + BColors.BG_ERR_TXT
                + "OnixD detected multiple Ctrl+C commands. Exiting now..."
                + BColors.ENDC
            )
            sys.exit(1)
...
tool_names = [
    ...
    [
        "dnsrecon",
        "DNSRecon - Attempts Multiple Zone Transfers on Nameservers.",
        "dnsrecon",
        1,
    ],
    # 11
    ...
    [
        "sslyze_hbleed",
        "SSLyze - Checks only for Heartbleed Vulnerability.",
        "sslyze",
        1,
    ],
    # 17
    ...
    [
        "sslyze_resum",
        "SSLyze - Checks for Session Resumption Support with [Session IDs/TLS Tickets].",
        "sslyze",
        1,
    ],
    # 26
    ...
    # 27
    [
        "golismero_dns_malware",
        "Golismero - Checks if the domain is spoofed or hijacked.",
        "golismero",
        1,
    ],
    # 28
    [
        "golismero_heartbleed",
        "Golismero - Checks only for Heartbleed Vulnerability.",
        "golismero",
        1,
    ],
    # 29
    [
        "golismero_brute_url_predictables",
    ...
    # 30
    [
        "golismero_brute_directories",
        "Golismero - BruteForces for certain directories on the Domain.",
        "golismero",
        1,
    ],
    # 31
    ...
    [
        "golismero_nikto",
        "Golismero Nikto Scans - Uses Nikto Plugin to detect vulnerabilities.",
        "golismero",
        1,
    ],
    # 37
    [
        "golismero_brute_subdomains",
        "Golismero Subdomains Bruter - Brute Forces Subdomain Discovery.",
        "golismero",
        1,
    ],
    # 38
    ...
    [
        "dmitry_email",
        "DMitry - Passively Harvests Emails from the Domain.",
        "dmitry",
        1,
    ],
    # 41
...
tool_cmd = [
    ...
    ["host ", ""],
    # 2
    ["wget -O /tmp/OnixD_temp_aspnet_config_err --tries=1 ", "/%7C~.aspx"],
    # 3
    ["wget -O /tmp/OnixD_temp_wp_check --tries=1 ", "/wp-admin"],
    # 4
    ["wget -O /tmp/OnixD_temp_drp_check --tries=1 ", "/user"],
    # 5
    ["wget -O /tmp/OnixD_temp_joom_check --tries=1 ", "/administrator"],
    # 6
    ["uniscan -e -u ", ""],
    # 7
    ["wafw00f ", ""],
    ...
    # 75
    ["wget -O /tmp/OnixD_temp_aspnet_elmah_axd --tries=1 ", "/elmah.axd"],
    # 76
    ["nmap -p445,137-139 --open -Pn ", ""],
    # 77
    ["nmap -p137,138 --open -Pn ", ""],
    # 78
    ["wapiti ", " -f txt -o OnixD_temp_wapiti"],
    # 79
...
tool_status = [
    ...
    # 33
    [
        "Could not find any vulnerability!",
        1,
        proc_med,
        " <  4m",
        "xsser",
...
def get_parser():
    ...
    parser.add_argument("-u", "--update", action="store_true", help="Update OnixD.")
    parser.add_argument(
        "-s",
        "--skip",
        action="append",
        default=[],
        help="Skip some tools",
        choices=[t[0] for t in tools_precheck],
    )
    parser.add_argument(
        "-n",
        "--nospinner",
    ...
    parser.add_argument(
        "target", nargs="?", metavar="URL", help="URL to scan.", default="", type=str
    )
    return parser
...
random.shuffle(scan_shuffle)  # nosec B311
tool_names, tool_cmd, tool_resp, tool_status = zip(*scan_shuffle)
tool_checks = (
    len(tool_names) + len(tool_resp) + len(tool_status)
) / 3  # Cross verification incase, breaks.
...
if args_namespace.help or (not args_namespace.update and not args_namespace.target):
    ...
    logo()
    print("OnixD is updating....Please wait.\n")
    spinner.start()
    ...
    try:
        oldversion_hash = (
            subprocess.check_output(["sha1sum", "OnixD/te.py"])
            .split()[0]
            .decode()
            .strip()
        )
    ...
    subprocess.call(
        [
            "wget",
            "-N",
            "https://raw.githubusercontent.com/Digvijay8580/OnixD/refs/heads/main/te.py",
            "-O",
            "te.py",
        ],
    ...
    try:
        newversion_hash = (
            subprocess.check_output(["sha1sum", "te.py"]).split()[0].decode().strip()
        )
    except Exception:
        newversion_hash = ""
    if oldversion_hash == newversion_hash:
        ...
        print(
            "\t"
            + BColors.OKBLUE
            + "You already have the latest version of OnixScan."
            + BColors.ENDC
        )
    else:
        clear()
        print(
            "\t"
            + BColors.OKGREEN
            + "OnixScan successfully updated to the latest version."
            + BColors.ENDC
        )
    spinner.stop()
    sys.exit(1)
    ...
    [
        os.remove(f)
        for f in glob.glob(os.path.join(tempfile.gettempdir(), "Onixscan*"))
        if os.path.exists(f)
    ...
    while rs_avail_tools < len(tools_precheck):
        ...
        try:
            ...
            print(
                "\t"
                + BColors.BG_ERR_TXT
                + "OniScanner was terminated abruptly..."
                + BColors.ENDC
            )
            sys.exit(1)
        ...
        if (
            b"not found" in val
            or tools_precheck[rs_avail_tools][arg1] in args_namespace.skip
        ):
    ...
    if len(unavail_tools_names) == 0:
        print(
            "\t"
            + BColors.OKGREEN
            + "All Scanning Tools are available. Complete vulnerability checks will be performed by OnixD."
            + BColors.ENDC
        )
    else:
        print(
            ...
            + "Some of these tools "
            + BColors.BADFAIL
            + str(unavail_tools_names)
            + BColors.ENDC
            + BColors.WARNING
            + " are unavailable or will be skipped. OnixD will still perform the rest of the tests. Install these tools to fully utilize the functionality of OnixD."
            + BColors.ENDC
        )
    print(
        BColors.BG_ENDL_TXT
    ...
    print(
        BColors.BG_HEAD_TXT
        + "[ Preliminary Scan Phase Initiated... Loaded "
        + str(tool_checks)
    ...
    while tool < len(tool_names):
        print(
            "["
            + tool_status[tool][arg3]
            + tool_status[tool][arg4]
            + "] Deploying "
            + str(tool + 1)
            + "/"
            + str(tool_checks)
        ...
        temp_file = (
            os.path.join(tempfile.gettempdir(), "onixscan_temp_")
            + tool_names[tool][arg1]
        )
        cmd_base = tool_cmd[tool][arg1] + target + tool_cmd[tool][arg2]
        ...
        if run_test == 1:
            ...
            print(
                "\n"
                + BColors.WARNING
                + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit OnixScan.\n"
                + BColors.ENDC
            )
            rs_skipped_checks = rs_skipped_checks + 1
    ...
    date = (
        subprocess.Popen(["date", "+%Y-%m-%d"], stdout=subprocess.PIPE)
        .stdout.read()[:-1]
        .decode("utf-8")
    )
    ...
    if len(rs_vul_list) == 0:
        ...
        with open(vulreport, "a") as report:
            while rs_vul < len(rs_vul_list):
                ...
                report.write(vuln_info[arg2].replace("Onixscan", "OnixD"))
                report.write("\n------------------------\n\n")
                temp_report_name = (
                    os.path.join(tempfile.gettempdir(), "Onixscan_temp_")
                    + vuln_info[arg1]
            ...
            print(
                "\tComplete Vulnerability Report for "
                + BColors.OKBLUE
                + target
                + BColors.ENDC
                + " named "
                ...
                + vulreport
                + BColors.ENDC
                + " is available under the same directory OnixScan resides."
            )
    ...
    print(
        "\tTotal Number of Vulnerability Checks        : "
        + BColors.BOLD
        + BColors.OKGREEN
        + str(len(tool_names))
        + BColors.ENDC
    ...
    print(
        "\tTotal Number of Vulnerability Checks Skipped: "
        + BColors.BOLD
        + BColors.WARNING
        + str(rs_skipped_checks)
        + BColors.ENDC
    )
    print(
    ...
    print(
        "\tTotal Time Elapsed for the Scan             : "
        + BColors.BOLD
        + BColors.OKBLUE
        + display_time(int(rs_total_elapsed))
        + BColors.ENDC
    )
    print("\n")
    print(
        "\tFor Debugging Purposes, You can view the complete output generated by all the tools named "
        + BColors.OKBLUE
        + debuglog
        + BColors.ENDC
    ...
    [
        os.remove(f)
        for f in glob.glob(os.path.join(tempfile.gettempdir(), "Onixscan_te*"))
        if os.path.exists(f)
    ]  # Clearing previous scan files
